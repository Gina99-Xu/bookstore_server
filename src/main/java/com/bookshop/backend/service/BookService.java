...
import com.bookshop.backend.dao.HistoryRepository;
...
import com.bookshop.backend.entity.History;
...
public class BookService {
	...
	public Book checkoutBook(String userEmail, Long bookId) throws Exception {
		...
		for (Checkout check : currentBooksCheckedout) {
			Date d1 = sdf.parse(check.getReturnDate());
		...
		if ((userPayment != null && userPayment.getAmount() > 0) || (userPayment != null && bookNeedsReturned)) {
			throw new Exception("Outstanding fees or books need to be returned!");
		}
	...
	public List<ShelfCurrentLoansResponse> currentLoans(String userEmail) throws Exception {
		...
		for (Checkout checkoutItem : checkoutList) {
			bookIdList.add(checkoutItem.getBookId());
		}
		...
		for (Book book : books) {
			Optional<Checkout> checkout = checkoutList.stream()
					.filter(item -> Objects.equals(item.getBookId(), book.getId())).findFirst();
